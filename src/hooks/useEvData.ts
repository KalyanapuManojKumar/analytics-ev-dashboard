...
export function useEvData() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.year != null) {
      list = list.filter((r) => r.modelYear === filters.year);
    }
    if (filters.make != null && filters.make !== "") {
      list = list.filter((r) => r.make === filters.make);
    }
    if (filters.model != null && filters.model !== "") {
      list = list.filter((r) => r.model === filters.model);
    }
    if (filters.vehicleType != null && filters.vehicleType !== "") {
      list = list.filter((r) => r.electricVehicleType === filters.vehicleType);
    }
    if (filters.county != null && filters.county !== "") {
    ...
    if (filters.state != null && filters.state !== "") {
      list = list.filter((r) => r.state === filters.state);
    }
    return list;
  }, [rawData, filters]);

  const summary: EVSummary = useMemo(
    () => computeSummary(filteredData),
    [filteredData],
  );
